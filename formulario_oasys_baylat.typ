...
Die finale Modellauswahl erfolgt kriterienbasiert nach Genauigkeit, Interpretierbarkeit und Eignung für Edge-Hardware (Raspberry Pi). Die UdC leitet Probenbereitstellung, Alterungsversuche und Standardanalytik. Das DIT verantwortet Modellierung, physikalische Verlustfunktionen und XAI-Validierung.
...
Frente a la quimiometría estática convencional (PLSR, SVR) y el alto costo de análisis cromatográficos continuos, el proyecto implementa un monitoreo ágil y no destructivo: lotes de café se someten a envejecimiento acelerado (25, 40 y 60 °C), adquiriendo espectros ATR-FTIR longitudinales acoplados a parámetros fisicoquímicos normatizados (AOAC: humedad, acidez titulable, peróxidos lipídicos y degradación de ácidos clorogénicos).
...
La selección de la mejor arquitectura se basa en precisión, explicabilidad y bajo costo computacional en hardware de borde (Raspberry Pi). La UdC lidera el envejecimiento, adquisición FTIR y analítica estándar. El DIT lidera el diseño algorítmico, formulación física y explicabilidad (XAI).
...
- Ziel 1 (UdC): Etablierung eines standardisierten Versuchsprotokolls zur kontrollierten akzelerierten Alterung von kolumbianischem Spezialitätenkaffee (25 °C, 40 °C, 60 °C; 60% und 75% r.F.) unter kontinuierlicher ATR-FTIR-Spektrenakquise und paralleler nasschemischer Bestimmung normierter AOAC-Referenzwerte (Feuchte, Wasseraktivität, Säuregrad, Peroxidzahl, Gesamtpolyphenole/Chlorogensäuren).
...
- Ziel 3 (Gemeinsam): Kriterienbasierte Pareto-Auswahl der optimalen Modellarchitektur (Validierungsgenauigkeit, chemische Kausalität der Bandenattribution mittels XAI, Inferenzfähigkeit auf Edge-Hardware wie Raspberry Pi).
...
- AP2 (Monate 3–8, UdC): Durchführung der akzelerierten Lagerung, kontinuierliche ATR-FTIR-Scans und AOAC-Analytik. Meilenstein M2: Vollständiger Kaffeespektraldatensatz fertiggestellt.
- AP3 (Monate 6–11, DIT): Modelltraining, 3-Ebenen-Benchmark und XAI-Attributionsanalyse. Forschungsaufenthalt des UdC-Nachwuchswissenschaftlers in Deggendorf (Monat 8–9). Meilenstein M3: Pareto-optimale Modellarchitektur identifiziert.
...
- Objetivo 1 (UdC): Estandarizar un protocolo de envejecimiento acelerado en café especial (25 °C, 40 °C, 60 °C; 60% y 75% HR) con adquisición longitudinal de espectros ATR-FTIR y medición de parámetros normatizados AOAC (humedad, actividad de agua, acidez titulable, peróxidos lipídicos, polifenoles totales y ácidos clorogénicos).
...
- Objetivo 3 (Conjunto): Seleccionar la arquitectura óptima mediante optimización multiobjetivo de Pareto (error de generalización, coherencia mecanicista de bandas espectrales mediante XAI y viabilidad computacional en dispositivos de borde tipo Raspberry Pi).
...
El grupo del Prof. Dr. Ricardo Vivas (UdC, Facultad de Ciencias Exactas y Naturales) cuenta con sólida trayectoria en química analítica, modelado computacional y aseguramiento de calidad agroalimentaria. Lidera el diseño experimental, envejecimiento acelerado y analítica de referencia.
...
El presupuesto financia directamente la cooperación presencial: (1) Misión de la Prof. Dr. Suárez a Cartagena para impartir un workshop intensivo de IA aplicada a espectroscopía para semilleros y posgrados de la UdC y calibrar protocolos de laboratorio; (2) Estancia de 3 semanas de un joven investigador de la UdC en Deggendorf para transferencia metodológica y entrenamiento conjunto de modelos.

6. Plan de trabajo, hitos y proyección futura
El plan de 12 meses se organiza en 4 Paquetes de Trabajo (WP):
- WP1 (Meses 1–4, UdC/DIT): Ajuste de protocolos, acondicionamiento de lotes de café y Workshop 1 en Cartagena con la Prof. Dr. Suárez (Mes 2). Hito H1: Protocolo de alteración y datos piloto establecidos.
- WP2 (Meses 3–8, UdC): Envejecimiento acelerado, mediciones continuas ATR-FTIR y analítica AOAC. Hito H2: Base de datos longitudinal consolidada.
- WP3 (Meses 6–11, DIT): Entrenamiento de modelos, benchmarking factorial y estancia del investigador junior de UdC en Deggendorf (Mes 8–9). Hito H3: Arquitectura Pareto-óptima identificada.
...
- AP2 (Monate 3–8, UdC): Akzelerierte Alterungsversuche (25 °C, 40 °C, 60 °C; 60% und 75% r.F.) über 180 Tage mit periodischer ATR-FTIR-Erfassung und nasschemischer Standardanalytik. Konsolidierung der longitudinalen Spektraldatenbank. Meilenstein M2 (Monat 8): Vollständiger Kaffeedatensatz fertiggestellt.
- AP3 (Monate 6–11, DIT/UdC): Algorithmenentwicklung und Benchmarking am DIT. Implementierung der 3 Modellstufen: klassische Chemometrie (PLSR, SVR), datengetriebenes Deep Learning (1D-CNN) und Physik-informierte Netze (PINN mit Arrhenius-Straftermen) sowie exploratives Screening (Mamba SSM, Transformers). 3-wöchiger Forschungsaufenthalt eines UdC-Nachwuchswissenschaftlers am KI-Campus Deggendorf (Monat 8–9). Meilenstein M3 (Monat 9): Identifikation der Pareto-optimalen Modellarchitektur (Genauigkeit, Kausalität, Edge-Inferenz auf Raspberry Pi).
...
#table(
  ...
  [M1], [AP1: Versuchsplanung], [UdC], [Beschaffung und botanische Verifizierung von kolumbianischem Spezialitätenkaffee (Arabica); Kalibrierung der Klimakammern (25 °C, 40 °C, 60 °C; 60%, 75% r.F.).],
  ...
  [M3], [AP2: Kinetik-Start], [UdC], [Start der akzelerierten Kaffeelagerung; Baseline-Scans ($t_0$) und periodische FTIR-Messungen ($t_1$); Bestimmung von Feuchte, aw, Säuregrad und Lipidperoxiden (AOAC 965.33).],
  ...
  [M8–9], [AP3: Mobilität & Tuning], [DIT/UdC], [*Forschungsaufenthalt (Präsenz in Deggendorf):* 3-wöchiger Aufenthalt eines UdC-Nachwuchswissenschaftlers am DIT für gemeinsames Training und XAI-Attributionsanalyse. *Meilenstein M2: Kaffeedatenbank konsolidiert*.],
  [M10], [AP3: Architekturauswahl], [DIT/UdC], [Multikriterielle Pareto-Entscheidungsfindung (Genauigkeit, chemische Kausalität, Inferenzlatenz < 2 s auf Edge-Hardware Raspberry Pi). *Entregable D3.1: Benchmark- und XAI-Bericht*. *Meilenstein M3: Optimale Architektur identifiziert*.],
...
#table(
  ...
  [Reise 1], [München -> Cartagena (UdC)], [Monat 2 (10 Tage)], [Prof. Dr. Isabel Suárez (DIT)], [*Workshop an der UdC:* Leitung des bilateralen KI- und Spektroskopie-Workshops für Nachwuchskräfte; Kalibrierung der Alterungskammern und Messprotokolle.],
